...
export async function load(file_url?: Blob | string): Promise<Tournament> {
    if (!file_url) {
        ...
        if (resp.ok) {
            ...
            const tournament = await deserializeTournament(data); // TODO binary file?
        ...
        return new Promise((resolve, reject) => {
            ...
            reader.addEventListener("loadend", async () => {
                try {
                    ...
                    const data = typeof reader.result === 'object' ? new TextDecoder().decode(reader.result) : reader.result;
                    const tournament: Tournament = JSON.parse(data, reviverDates);
                    tournament._url = (file_url as File).name; //TODO missing path
                    //this.selection.select(tournament, ModelType.Tournament);
                    resolve(await reviveTournament(tournament));
...
export async function save(tournament: Tournament, url?: string) {
    ...
    if (!url) {
        ...
        window.localStorage.setItem('tournament', serializeTournament(tournament));
        return;
    }
...
export function getRegisteredPlayers({ rank }: DataType, allPlayers: Player[], event: TEvent, drawMinRank: RankString, drawMaxRank: RankString): Player[] { //GetJoueursInscrit
...
export function sortPlayers({ rank }: DataType, players: Array<Player | number>): void {
