...
const indexes = new WeakMap<any_[],Map<string,IIndex>>();

/**
 * Returns the index of an object in the array. Or -1 if not found.
*/
export function indexOf<T>(array: T[], member: StringKey<T>, value: any_, error?: string): number {
 ...
 */
export function by<T>(array: T[], member: StringKey<T>, value: any_, error?: string): T | undefined {
...
export function byId<T extends {id: string}>(array: T[], value: any_): T | undefined;
export function byId<T extends {id: string}>(array: T[], value: any_, error: string): T;
export function byId<T extends {id: string}>(array: T[], value: any_, error?: string): T | undefined {
    const i = indexOf(array, "id" as any_, value, error);
    if (error && !array[i]) {
        throw new Error(error);
    }
    return array[i];
